...
pub fn decrypt(encrypted_data: EncryptionResult, parent_key: &Key) -> Result<Vec<u8>, Box<dyn Error>> {
...
mod tests {
  ...
  use crate::protos::encryption_result::EncryptionResult;
  use std::error::Error;
  use std::str::FromStr;
  ...
  fn it_works() -> Result<(), Box<dyn Error>> {
    ...
    let deserialized_result: EncryptionResult = protobuf::parse_from_bytes(&serialized)?;
